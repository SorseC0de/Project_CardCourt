...
struct CourtView: View {
    ...
    /// **The ball reaching somebody's hands**, which the controller owns — see
    /// `GameController.caught`.
    var caught: GameController.Catch?
        ...
        AnyView(Group {
            ...
            return VStack(spacing: 3) {
                PlayerFigure(
                    ...
                    // **Only the man it names**, and only on the instant the ball
                    // actually reached him — see `GameController.caught`. The floor kept
                    // its own clock for this and the two drifted.
                    caughtAt: caught?.seat == seat ? caught?.at : nil,
