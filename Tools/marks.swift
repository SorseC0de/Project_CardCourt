...
//
// Compiles `MarkTable.swift` itself rather than a copy of it, so it cannot drift.
...
var table: [String: Spot] = [:]
func place(_ sheet: String, _ frame: Int, _ eye: Mark, _ spot: Spot) {
    table["\(sheet)/\(frame)/\(eye.rawValue)"] = spot
}

place("Player_front", 0, .near, Spot(x: 0, y: 1))
place("Player_front", 0, .far, Spot(x: 0, y: 1))
for f in 0..<4 {
    place("Player_front_spinball", f, .near, Spot(x: 1, y: 0))
    place("Player_front_spinball", f, .far, Spot(x: 1, y: 0))
}
for f in 0..<16 {
    place("Player_Run_Look", f, .near, Spot(x: 2, y: 0))
    place("Player_Run_Look", f, .far, Spot(x: 2, y: -1))
}
place("Player_Run_Look", 7, .far, Spot(x: 2, y: -2))
place("Player_Run_Look", 8, .far, Spot(shown: false))
place("Player_right", 0, .near, Spot(x: -1, y: 0))
place("Player_right", 0, .far, Spot(shown: false))
// Half-done on purpose: three of sixteen cells placed, so no wildcard may be written.
for f in 0..<3 { place("Player_Wave", f, .near, Spot(x: 3, y: 0)) }

let source = MarkTable.source(from: table, sheets: sheets)
print(source)
...
for sheet in sheets {
    for frame in 0..<sheet.frames {
        for eye in Mark.allCases {
            let wanted = MarkTable.spot(sheet.name, frame: frame, eye: eye, in: table)
            let got = MarkTable.spot(sheet.name, frame: frame, eye: eye, in: parsed)
            if wanted != got {
                bad += 1
