...
| **Boarder Court** | Varena | Roswell Reach off your *own* miss: the shooter's Rebound bid counts as one card more. Still needs a bid of at least one. Retires Off the Backboard | TBC |
| **Blight Ball** | Variaball | Injuries travel with the ball — pass the token and your Injury *cards* go with it. New injuries join the pile, Katamari-style, so it escalates. Discarding the ball takes the pile with it, so anyone can end it by spending a Variaball — nobody wants to be the one who does | TBC |
| **Dim Dome** | Varena | The SHOT number is hidden from everyone but the ball holder. On a table you flip the % device face down | TBC |
| **Tri-hard Tiling** | Varena | Hand limit 3. *Tri* for the number, *try-hard* for the joke, tiling for the anchor — the card teaches its own rule | TBC |
| **Policeum** | Varena | Referees do not leave once triggered, so their restrictions run until the floor changes. The existing 3-ref cap bounds it. Police + policy + coliseum | TBC |
| **Kiddie Court** | Varena | A shrunk floor: no threes, every make counts 2. SHOT +10%, and dunks a further +10% on the low rim. Shuts off every `upgradesToThree` card | TBC |
| **Bench Ball** | Variaball | Receiving it by *pass* benches you: no draw, no turn, straight to the inbound. Inbounds are not passes, so the ball keeps moving and the chain cannot cascade. Retires Benched | TBC |
| **Dishtracting Ball** | Variaball | Receiving it costs you a discard, after the draw for turn. The inverse of Dishcount Ball. Retires Crowd Noise | TBC |
| **Recharging Resin** | Varena | Everyone refills to a hand of 5 at the start of their possession. Electric-themed, and the direct opponent of Tri-hard Tiling's cap of 3. Retires Designed Play | TBC |
| **Contact Court** | Varena | Being clamped sends you to the line for one free throw. The Clamp still lands. The expensive-clamping floor, against Smacktop's cheap one. Retires Foul | TBC |
...
| **Hand Ball** | Variaball | Passing **swaps** hands: yours goes with the ball, theirs comes back. The sprite shrinks to handball size, which is the whole visual. The space is deliberate — it keeps the `[X] Ball` convention while sitting one space from the real sport. Retires Traded Mid-Game | TBC |
| **Polypaypylene** | Varena | Making a shot pays you cards. *Polypropylene* is the real material in modular sport-court tiles, with *pay* in the middle of it. Retires All-Swissh Selection. Count on a make: TBC | TBC |
| **Foot Ball** | Variaball | **Moves and Passes are not consumed** — playing one locks it (`#[Lock]`, existing keyword) instead of discarding it, and the lock clears the moment your possession ends. A straight buff to ball movement: passing costs nothing, so you pass. Art is an orange soccer ball with the black hexagons | TBC |
| **Recharge Rock** | Variaball | Double your draw for turn. The electric ball, twin to Recharging Resin — they sit in different slots, so they stack. Retires All Star Selection | TBC |
| **Recoverena** | Varena | On play, discard all injuries. Standing, a new injury is discarded instead of applying and that player draws 1. Recover + Varena. Retires Hit the Bike | TBC |
...
| **Shufflebag Ball** | Variaball | Persistent. Every possession, at the start of the active player's turn: shuffle their hand into the deck, draw the same number back, then proceed to their normal draw phase on top of that. Repeats every turn it's out — full hand randomization plus the normal draw, every single possession | TBC |
| **Carousel Court** | Varena | Hands rotate one seat in a declared direction, every possession. Retires Trade Deadline | TBC |
| **Traderous Tarmac** | Varena | You may reassign a clamp that is on *you* to another player. Counter-clamp, like Spin Move: prompts at the start of your turn. A clamp with no lock/discard (e.g. Contest's SHOT debuff) can be declined at the prompt and reassigned later, after your draw phase. A clamp that locks or discards (Double-Team, Triple-Team, Full-Court Press) must be countered at the prompt or it lands — decline and you eat it | TBC |
| **Mop Maple** | Varena | All players shuffle their hands into the deck and draw the same number of cards they had. Everyone redraws, free. Retires Floor Cleanup | TBC |
| **Clearcoat Court** | Varena | At the start of every possession: clear all referees, clamps, injuries, waiting effects (armed Whistles), and intangibles. The universal reset floor — every standing status type in the game, wiped each turn. Retires Official Timeout | TBC |
| **Malice Palace** | Varena | At the start of your possession, before your draw phase: discard your whole hand. Palace is real NBA arena vocabulary; the name also nods at the actual brawl. Retires Huge Altercation | TBC |
| **Turnstile Tile** | Varena | The shooter's SHOT alternates +25% / −25% each turn it's out. Visual: the floor recolors red/green each flip, Mario 3D flip-panel style — same cheap-recolor trick as Blight Ball's accumulation. Retires Home Court Advantage and Away Game | TBC |
| **Bag'n Ball** | Variaball | Draw 1 on activation. SHOT is overridden to equal the number of cards in your bag (hand). Art is styled after a Dragon Ball. Retires In The Zone | TBC |
| **Roleplayer Polymer** | Varena | At the start of each possession, everyone except the turn player draws 1. Doesn't lock anyone out — the excluded seat is whoever's shooting *this* turn, and that rotates every possession. Retires Role Player | TBC |
| **Variaball Vinyl** | Variaball | The draw-for-turn goes to a random player each possession instead of the active player — can be them. Vinyl is a genuine cheap-ball material, matching "generic ball, anybody's." Not tied to any of the original 30 — a new addition | TBC |
| **Gravi-Gym** | Varena | SHOT −10%, no dunks. The inverse of Kiddie Court: one floor kills the arc and rewards the rim, this one kills the rim entirely and makes everything harder | TBC |
| **Blaze Ball** | Variaball | SHOT +10% each time it's passed, stacking. Fires on its own flat delta regardless of whatever else that pass is doing to SHOT — ignores other pass modifiers entirely | TBC |
| **Snow Ball It** | Variaball | SHOT −10% each time it's passed, stacking. Blaze Ball's opposite number, same independence from other pass modifiers. Retires Ice Wrap's freed name | TBC |
| **Vintage Varnish** | Varena | Shot clock 14. No threes. No Variaballs may be played while it's out. 1 Intangible per player. The retro floor — the only card that reaches into the *other* slot and shuts it off entirely | TBC |
| **S.O.S — Sell-Out Stadium** | Varena | A card that would shoot a three can instead be shot as a two, at double SHOT, keeping every other printed effect. Named for defenders selling out on the closeout, faked, and beaten for an easy two. The risk/reward mirror of Kiddie Court — choice instead of a flat rule | TBC |
| **Grayvstone** | Varena | Each possession, the ball is replaced by the last ball in the Variaball discard pile. Grave + gray + stone, softened from a literal graveyard read; the floor recolors gray to match. Reaches into the ball slot the way Vintage Varnish does. *Open: what happens the first possession, before anything has been discarded* | TBC |
| **Con-crete** | Varena | Move cards lower SHOT −10%. Con as in cost, concrete as the genuinely brutal real surface — hard on the joints | TBC |
| **Brick Ball** | Variaball | SHOT is overridden to a flat 25%, always. No overlap with Med Ball (a ceiling) or Bag'n Ball (a formula) — a third, distinct kind of override | TBC |
| **Spazzphalt** | Varena | SHOT is overridden to a random value 0–100 in steps of 5, regardless of what ball is in play — the court dictates the terms. Loses only to an Intangible override. "Shooting on me? These are the rules" | TBC |
| **Frostbite Finish** | Varena | Playing a Move card requires discarding another card. If the Move is your whole hand, you cannot play it — there is nothing left to discard. The icy mirror of Foot Ball's buff; floor finish is the real term for a court's coating, so the ice literally is the finish | TBC |
| **Tick-Tock Tile** | Varena | Playing any card also ticks the shot clock, on top of its normal countdown and normal per-possession reset. Scoped to one floor instead of a blanket rule, which sidesteps the reset-behavior question the shelved version raised | TBC |
...
**Smacktop's wording is open.** Proposed: a single keyword — *Chippy*, real basketball
vocabulary for a rough, uncalled game — defined once as "each of a Clamp's numbers goes up
one step." One line on the Varena and nothing added to any Clamp face, which matters
because the faces are already tight. The alternative is a bespoke enhancement printed on
every Clamp.
...
- **Crowd Noise**, **Foul**, **Benched**, **Traded Mid-Game**, **Designed Play**, **MVP Vote**,
  ...
  **Fresh Ball**, **Trade Deadline**, **Floor Cleanup**, **Official Timeout**,
